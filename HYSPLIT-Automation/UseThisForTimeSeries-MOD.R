...
Titles <- c("Sensitivity to Stack Height",
...
for(i in 1:3) {
    ...
    Data_1 <- read.csv(paste(Locations[i], "_B", "_2012_0.1", sep = ""), header = TRUE)[,2 + PlotCol]
    Data_2 <- read.csv(paste(Locations[i], "_C", "_2012_0.1", sep = ""), header = TRUE)[,2 + PlotCol]
    Data_3 <- read.csv(paste(Locations[i], "_D", "_2012_0.1", sep = ""), header = TRUE)[,2 + PlotCol]
    Data_4 <- read.csv(paste(Locations[i], "_E", "_2012_0.1", sep = ""), header = TRUE)[,2 + PlotCol]
    ...
    plot <- ggplot(Data_Melted, aes(x = Day, y = value)) +
      geom_line(aes(color = variable), size = 1.0) + 
      facet_grid(variable ~ ., scales = "free_y") +
      ylim(0, max(max(Data_1), max(Data_2), max(Data_3), max(Data_4))) +
