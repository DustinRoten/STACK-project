...
# This script is responsible for metrics for the increased angular dispersion scenario ONLY.
...
for(i in 0:100) {
    ...
    Matrix_Origin_Dispersion <- GridDispersions2(Origin_Dispersion, Radial_Dispersion, Resolution, 1)
    Matrix_Radial_Dispersion <- GridDispersions2(Origin_Dispersion, Radial_Dispersion, Resolution, 2)
    Origin <- GridDispersions2(Origin_Dispersion, Radial_Dispersion, Resolution, "O")
    
    Matrix_Radial_Dispersion <- Matrix_Radial_Dispersion*(sum(Matrix_Origin_Dispersion)/sum(Matrix_Radial_Dispersion))
    
    MRSMeasure[i+1] <- (1/(2*sum(Matrix_Origin_Dispersion)))*sum(abs(Matrix_Origin_Dispersion - Matrix_Radial_Dispersion))*100
